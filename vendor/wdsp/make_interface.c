  ...
  for (i = 1; i < argc; i++) {
    ...
    for (;;) {
      ...
      if (strlen(line) > 4) {
        int pos = 4;
        if (line[4] == ' ') { pos++; }
        needle = strchr(line + pos, '{');
        if (needle) {
          *needle = 0;
          trimm(line, linesize);
          needle = strstr(line + pos, "()");
          if (needle) {
            *needle = 0;
            snprintf(shipout, sizeof(shipout), "%s(void)%s", line + pos, needle + 2);
          } else {
            snprintf(shipout, sizeof(shipout), "%s", line + pos);
          }
          printf("extern %s;\n", shipout);
          continue;
        }
      ...
      for (;;) {
        ...
        trimm(line, linesize);
        if (line[0] != 0) {
          char *empty_args = strstr(line, "()");
          if (empty_args) {
            *empty_args = 0;
            snprintf(shipout, sizeof(shipout), "%s(void)%s", line, empty_args + 2);
          } else {
