...
void teardown_nf_ws(NF_WS w) {
  if (!w) { return; }
  _aligned_free(w->t_new2);
  _aligned_free(w->t2);
  _aligned_free(w->pts2);
  _aligned_free(w->xs_sorted);
  _aligned_free(w->w_data);
  _aligned_free(w->new_knots);
  _aligned_free(w->seg_count);
  _aligned_free(w->seg_rms);
  _aligned_free(w->t_fold);
  _aligned_free(w->pts_fold);
  _aligned_free(w->t_work_b);
  _aligned_free(w->t_work_a);
  _aligned_free(w->pts_work_b);
  _aligned_free(w->pts_work_a);
  _aligned_free(w->t_in);
  _aligned_free(w->pts_in);
  _aligned_free(w->cv_mask);
  _aligned_free(w->inlier_mask);
  _aligned_free(w->t_params);
  _aligned_free(w->ordered);
  _aligned_free(w->pts_filtered);
  _aligned_free(w->lor_abs_dev);
  _aligned_free(w->lor_sorted);
  _aligned_free(w->lor_band_idx);
  _aligned_free(w->lor_band_res);
  _aligned_free(w->rp_t_old);
  _aligned_free(w->sr_seg_sse);
  _aligned_free(w->cv_res);
  _aligned_free(w->om_res);
  _aligned_free(w->fp_cyf);
  _aligned_free(w->fp_cxf);
  _aligned_free(w->fp_cy);
  _aligned_free(w->fp_cx);
  _aligned_free(w->fp_by);
  _aligned_free(w->fp_bx);
  _aligned_free(w->ls_L);
  _aligned_free(w->ls_Atb);
  _aligned_free(w->ls_AtA);
  _aligned_free(w->mat_Nfree);
  _aligned_free(w->mat_N);
  _aligned_free(w->nn_visited);
  _aligned_free(w->nn_order);
  _aligned_free(w->sp_iy);
  _aligned_free(w->sp_ix);
  _aligned_free(w->sp_ry);
  _aligned_free(w->sp_rx);
  _aligned_free(w->sp_ys);
  _aligned_free(w->sp_xs);
  _aligned_free(w);
}
