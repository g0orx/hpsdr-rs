...
// CHANGED (2026-09-08, real-hardware PureSignal investigation -- see
// memory/wdsp_210_port.md): was 0 (disabled) in the upstream source this
// was ported/re-ported from (both the original wdsp_210 snapshot and
// deskHPSDR's own bundled copy agree on 0 here) -- nurbs_fit.c's own
// internal library default for this same field is 2, i.e. upstream WDSP
// deliberately turns this OFF specifically for PureSignal's three curve
// fits, for reasons not established here. Enabling it (matching the
// library's own suggested default) is a targeted experiment based on
// concrete real-hardware diagnostic data: repeated `raw quality=0x10011`
// (magnitude fit CONDNUM) failures were traced to a small, consistent
// cluster of samples pinned to a narrow ~2-2.5e-5 env_RX floor
// (independent of the true two-tone envelope, which continues down to
// its own genuine ~0.00414 null depth) -- everywhere else the gain curve
// is nearly perfectly flat (~1.0, a very linear amp), so this is a
// tightly-localized cluster of degenerate points at the extreme low end
// of the x-range, not points scattered broadly enough for the GLOBAL
// outlier check above (PS_NF_OUTLIER_*, judged against the whole curve's
// variance) to reliably flag. LOCAL (per-x-band) outlier rejection is
// exactly the mechanism designed for this shape of problem -- splits the
// x range into PS_NF_LOCAL_OUTLIER_BANDS bands and computes a robust
// median/MAD residual check within each band independently, so the
// bottom band (where this cluster lives) gets judged against its own
// local statistics instead of the whole dataset's. Cheap to revert (this
// #define alone) if it doesn't help or causes a regression elsewhere.
// Reverted to upstream's own value (0, disabled) -- 2026-09-08
// real-hardware test showed zero effect (identical raw quality=0x10011
// before/after), so this genuine deviation from both reference WDSP
// snapshots isn't earning its keep. See memory/wdsp_210_port.md: the
// most likely reason it didn't help is that the floor-affected sample
// population is probably too LARGE a fraction of its own local x-band
// for a median/MAD check to identify as anomalous (robust statistics
// break down once contamination approaches/exceeds roughly half of the
// window being judged) -- being investigated with a population-count
// diagnostic instead of guessing at more fit-level tuning knobs.
